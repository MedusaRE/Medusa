...
\begin{document}
	...
	* Since the writing of \texttt{README.md}, I have changed my opinion slightly
	...
	As was stated in the \textit{Introduction} section, the \textit{Medusa}
	project was started under a different name, \textit{xp$^{DBG}$}.
	\textit{xp$^{DBG}$} stands (or rather, stood) for "cross (X) Platform
	DeBuGger", which references the project's goal to create a cross platform
	suite for software and hardware research. At the start of the project, the
	goal was not necessarily to create a full suite. In fact, before
	\textit{xp$^{DBG}$}, 
\end{document}
